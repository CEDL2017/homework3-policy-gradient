...
class CategoricalPolicy(object):
    def __init__(self, in_dim, out_dim, hidden_dim, optimizer, session):
        ...
        """
        # YOUR CODE HERE >>>>>>
        fc1 = tf.contrib.layers.fully_connected( inputs = self._observations, num_outputs = hidden_dim, activation_fn = tf.nn.tanh )
        fc2 = tf.contrib.layers.fully_connected( inputs = fc1, num_outputs = out_dim, activation_fn = tf.nn.softmax )
        probs = fc2
        # <<<<<<<<
